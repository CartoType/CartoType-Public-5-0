...
/* Palm OS doesn't supply or need the following headers */
#define LACKS_UNISTD_H
#define LACKS_SYS_PARAM_H
#define LACKS_SYS_MMAN_H
...
#if (defined(__PALMOS_TRAPS__) || defined (__PALMOS_ARMLET__) || defined(WIN32))
...
/* 
  ...
  Emulation of sbrk for Palm OS.
  Allocate large initial block and subsequently smaller blocks.
  If the default size can not be allocated allocate the largest
  possible block.
  User must call shutdown routine.
...
#include <PalmTypes.h>
#include <PalmOSGlue.h>
#include <cartotype_malloc.h>

#define MAX_BLOCKS 100
int g_next_block = 0;
MemPtr g_blocks[MAX_BLOCKS];
MemPtr sbrk_top = 0;
long g_min_block_size = 0;
long g_next_block_size = 0;
...
static void* sbrk (ptrdiff_t size) 
	...
	if (!g_min_block_size)
		{
		// First call: set some parameters based on heap size.
		long heap_size = MemHeapSize(0);
		if (FIRST_BLOCK_SIZE * 2 < heap_size)
			{
			g_min_block_size = FIRST_BLOCK_SIZE;
			g_next_block_size = MINIMUM_BLOCK_SIZE;
			}
		else
			{
			g_min_block_size = heap_size / 2;
			g_next_block_size = MINIMUM_BLOCK_SIZE_SMALL_HEAP;
			}
		}
	alloc_size = g_min_block_size;
#ifdef TRACE
	FILE* f = fopen("sbrk.log","a");
	char text[128];
	sprintf(text,"sbrk %ld\r\n",size);
	fwrite(text,1,strlen(text),f);
#endif
	if (size > 0)
		{
		if (g_next_block == MAX_BLOCKS)
			{
#ifdef TRACE
			sprintf(text,"max blocks reached %ld",g_next_block);
			fwrite(text,1,strlen(text),f);
			fclose(f);
#endif			
			return result;
		...
		if (alloc_size < size)
			alloc_size = size;
#ifdef TRACE
		sprintf(text,"alloc %ld\r\n",alloc_size);
		fwrite(text,1,strlen(text),f);
#endif						
		g_blocks[g_next_block] = MemGluePtrNew(alloc_size + MALLOC_ALIGNMENT);
#ifdef TRACE
		sprintf(text,"alloc result %ld\r\n",(long)g_blocks[g_next_block]);
		fwrite(text,1,strlen(text),f);
#endif		
				if (g_blocks[g_next_block] == 0)
			{
			/* Allocation failed, find maximum heap block size and use that. */
			unsigned long max;
			unsigned long free;
			MemHeapFreeBytes(0,&free,&max);
			if (max >= size)
				{
				alloc_size = max - MALLOC_ALIGNMENT;
				g_blocks[g_next_block] = MemGluePtrNew(max);
				}
			if (g_blocks[g_next_block] == 0)
#ifdef TRACE
				sprintf(text,"alloc failed size %ld\r\n",max);
				fwrite(text,1,strlen(text),f);
				fclose(f);
#endif				
				return result;
			}
		result = (void *)((((CHUNK_SIZE_T)g_blocks[g_next_block++]) + MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK);
#ifdef TRACE
		sprintf(text,"return result %ld\r\n",(long)result);
		fwrite(text,1,strlen(text),f);
#endif		
		sbrk_top = (char *)result + alloc_size;
#ifdef TRACE
		sprintf(text,"alloc_s %ld\r\n",alloc_size);
		fwrite(text,1,strlen(text),f);
#endif		
		g_min_block_size -= alloc_size;
		if (g_min_block_size < g_next_block_size)
			g_min_block_size = g_next_block_size;
#ifdef TRACE
		fclose(f);
#endif				
	    return result;
...
void sbrk_init(long aMinimumFirstBlockSize,long aMinimumBlockSize)
	{
	g_min_block_size = aMinimumFirstBlockSize;
	g_next_block_size = aMinimumBlockSize;
	}

void sbrk_shutdown()
	{
	int i;
	for (i = 0; i < g_next_block; ++i)
		MemPtrFree(g_blocks[i]);
	}
