...
template<size_t aPointCount> class TFixedSizeContour: public MPath
    {
    public:
    TFixedSizeContour(bool aClosed): iClosed(aClosed) { }
    size_t Contours() const override { return 1; }
    void GetContour(size_t /*aIndex*/,TContour& aContour) const override { aContour = TContour(&iPoint[0],aPointCount,iClosed); }
    operator TContour() const { return TContour(&iPoint[0],aPointCount,iClosed); }
    
    bool iClosed;
    std::array<TOutlinePoint,aPointCount> iPoint;
    };
