...
Copyright (C) 2015-2017 CartoType Ltd.
...
class TMapState
    ...
    TFixedSizeContour<4> m_view_in_map_coords = TFixedSizeContour<4>(true);
...
class CVectorTileHelper
    ...
    */
    ...
    create the data needed to draw the label bitmaps: usually that will be a texture atlas.
    It may return null if label bitmaps are not used.
    The framework is supplied so that label points can be converted to map coordinates if necessary.
    */
    virtual std::unique_ptr<CLabelDrawData> CreateLabelDrawData(CFramework& /*aFramework*/,const std::vector<CPositionedLabel>& /*aLabelArray*/,const TViewState& /*aViewState*/) { return nullptr; }
...
class CVectorTileServer: public MFrameworkObserver
    ...
    std::unique_ptr<CLabelDrawData> CreateLabelDrawData(CFramework& aFramework,const std::vector<CPositionedLabel>& aLabelArray,const TViewState& aViewState);
